...
class PPO {
  ...
  static auto update(ActorCritic& ac, torch::Tensor& states, torch::Tensor& actions, torch::Tensor& log_probs,
                     torch::Tensor& returns, torch::Tensor& advantages, OPT& opt, uint steps, uint epochs,
                     uint mini_batch_size, double beta, double clip_param = .2) -> void;
};
...
auto PPO::returns(VT& rewards, VT& dones, VT& vals, double gamma, double lambda) -> VT {
  ...
  for (
      uint i = rewards.size();
      i--
      > 0;) // inverse for loops over unsigned:
            // https://stackoverflow.com/questions/665745/whats-the-best-way-to-do-a-reverse-for-loop-with-an-unsigned-index/665773
  {
    // Advantage.
...
auto PPO::update(ActorCritic& ac, torch::Tensor& states, torch::Tensor& actions, torch::Tensor& log_probs,
                 torch::Tensor& returns, torch::Tensor& advantages, OPT& opt, uint steps, uint epochs,
                 uint mini_batch_size, double beta, double clip_param) -> void {
  for (uint e = 0; e < epochs; e++) {
    // Generate random indices.
    torch::Tensor cpy_sta = torch::zeros({mini_batch_size, states.size(1)});
    torch::Tensor cpy_act = torch::zeros({mini_batch_size, actions.size(1)});
    torch::Tensor cpy_log = torch::zeros({mini_batch_size, log_probs.size(1)});
    torch::Tensor cpy_ret = torch::zeros({mini_batch_size, returns.size(1)});
    torch::Tensor cpy_adv = torch::zeros({mini_batch_size, advantages.size(1)});
