...
int BT_setEV3name(const char* name) {
  ...
  len = strlen(name);
  if (len > 12) {
    fprintf(stderr, "BT_setEV3name(): The input name string is too long - 12 characters max, no white spaces or "
                    "special characters\n");
    return (-1);
  }
...
int BT_play_tone_sequence(const int tone_data[50][3]) {
  ...
  for (int i = 0; i < 50; i++) {
    if (tone_data[i][0] == -1 || tone_data[i][1] == -1)
      break;
    if (tone_data[i][0] < 20 || tone_data[i][0] > 20000) {
      fprintf(stderr, "BT_play_tone_sequence():Tone range must be in 20Hz-20KHz\n");
      return (0);
    }
    if (tone_data[i][1] < 1 || tone_data[i][1] > 5000) {
      fprintf(stderr, "BT_play_tone_sequence():Tone duration must be in 1-5000ms\n");
      return (0);
    }
...
int BT_motor_port_start(char port_ids, char power) {
  ...
  void* p;
  unsigned char* cp;
  unsigned char cmd_string[15] = {0x0D, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0xA4,
                                  0x00, 0x00, 0x81, 0x00, 0xA6, 0x00, 0x00};
  //                          |length-2| | cnt_id | |type| | header |  |set power| |layer|  |port ids|  |power| |start|
  //                          |layer| |port id|
...
int BT_motor_port_speed(char port_ids, char speed) {
  ...
  unsigned char* cp;
  char reply[1024];
  unsigned char cmd_string[15] = {0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA5,
                                  0x00, 0x00, 0x81, 0x00, 0xA6, 0x00, 0x00};
...
int BT_turn(char lport, char lpower, char rport, char rpower) {
  ...
  void* p;
  unsigned char* cp;
  unsigned char cmd_string[20] = {0x12, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0xA4, 0x00, 0x00,
                                  0x81, 0x00, 0xA4, 0x00, 0x00, 0x81, 0x00, 0xA6, 0x00, 0x00};
  //                          |length-2| | cnt_id | |type| | header |  |set power| |layer|  |lport id|  |power|  |set
  //                          power| |layer| |rport id| |power|     |start|  |layer| |port ids|
...
int BT_timed_motor_port_start(char port_id, char power, int ramp_up_time, int run_time, int ramp_down_time) {
  ...
  void* p;
  unsigned char* cp;
  unsigned char cmd_string[22] = {0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x81,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  //                          |length-2| | cnt_id | |type|   |header|    |cmd| |layer| |port ids|  |power|      |ramp
  //                          up|      |run|           |ramp down|      |brake|
...
int BT_timed_motor_port_start_v2(char port_id, char power, int time) {
  ...
  unsigned char cmd[26] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA4, 0x00, 0x00, 0x81, 0x00, 0xA6,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA3, 0x00, 0x00, 0x00};
  //                     |length-2| |cnt_id|   |type| |header| |set_pwr| |layer| |port| |power|  |start| |layer| |port|
  //                     |wait| |LC2|    |time| |var addr| |ready| |var addr| |stop| |layer| |port_id| |break|
...
int BT_read_touch_sensor(char sensor_port) {
  ...
  char reply[1024];
  unsigned char* cp;
  unsigned char cmd_string[15] = {0x0D, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  //                          |length-2| | cnt_id | |type| | header |   |cmd|  |sensor cmd | |layer|  |port| |type|
  //                          |mode| |data set| |global var addr|
...
int BT_read_colour_sensor_RGB(char sensor_port, int RGB[3]) {
  ...
  unsigned char cmd_string[17] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00,
...
int BT_read_ultrasonic_sensor(char sensor_port) {
  ...
  unsigned char cmd_string[15] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  //                          |length-2| | cnt_id | |type| | header |   |cmd|  |sensor cmd | |layer|  |port| |type|
  //                          |mode| |data set| |global var addr|
...
int BT_read_gyro_sensor(char sensor_port, int angle_speed[2]) {
  ...
  unsigned char cmd_string[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  //                          |length-2| | cnt_id | |type| | header |   |cmd|  |sensor cmd | |layer|  |port| |type|
  //                          |mode| |data set| |global var addr|
