...
struct displayList* addVector(struct displayList* head, int x1, int y1, double dx, double dy, int length, double R,
                              double G, double B) {
  struct displayList* newNode;
  double l;
...
struct blob* id_coloured_blob2(struct RoboAI* ai, struct blob* blobs, int col) {
  ...
  while (p != NULL) {
    ...
    grayness = 0;
    if (fabs(p->R - p->G) / p->R < maxgray && fabs(p->R - p->G) / p->G < maxgray && fabs(p->R - p->B) / p->R < maxgray
        && fabs(p->R - p->B) / p->B < maxgray && fabs(p->G - p->B) / p->G < maxgray
...
void AI_main(struct RoboAI* ai, struct blob* blobs, void* state) {
  ...
  **************************************************************************/
  ...
  if (ai->st.state == 0 || ai->st.state == 100
      || ai->st.state == 200) // Initial set up - find own, ball, and opponent blobs
  {
    ...
    {                              // if robot identification is successful.
      ...
      fprintf(stderr,
              "Self-ID complete. Current position: (%f,%f), current heading: [%f, %f], blob direction=[%f, %f], AI "
              "state=%d\n",
